...
def json_safe(value: Any) -> Any:
    """Convert common Python values into JSON-compatible representations."""
...
class JobQueue:
    ...
    async def reserve_sync_slot(connection: Any) -> bool:
        ...
        await connection.execute(
            ...
            "hashtext('football-agent:sync-pipeline')"
            ")"
        )
        ...
        async with pool.connection() as connection:
            async with connection.transaction():
                if job_type == "sync_pipeline":
                    # Serialize manual and scheduled syncs, whose external
                    # side effects are idempotent but expensive and rate limited.
        ...
        async with pool.connection() as connection:
            async with connection.transaction():
                result = await connection.execute(
                    ...
                    """,
                    (
                        Jsonb(json_safe(result_payload or {})),
                        job.id,
                        job.owner_token,
                    ),
                )
        ...
        async with pool.connection() as connection:
            async with connection.transaction():
                result = await connection.execute(
                    ...
                    (
                        error[:4000],
                        Jsonb(json_safe(failure_payload or {})),
                        job.id,
                        job.owner_token,
                    ),
                )
