...
cd "$(dirname "$0")/.."
fixture_db="football_intelligence_schema_fixture"
export PGPASSWORD="$APP_DB_PASSWORD"
psql -h "$APP_DB_HOST" -p "$APP_DB_PORT" -U "$APP_DB_USER" -d postgres -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS ${fixture_db}"
...
APP_DB_NAME="$fixture_db" MIGRATIONS_ROOT=. python services/collector/scripts/migrate.py
