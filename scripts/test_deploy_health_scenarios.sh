...
if [[ "${1:-}" == "compose" ]]; then
  ...
  if [[ "$arguments" == *" config "* && "$arguments" == *" --format json "* ]]; then
    cat <<'JSON'
{"name":"football-agent","services":{"proxy":{"ports":[{"host_ip":"127.0.0.1","published":18088}],"networks":{"football_edge":{}}},"postgres":{"networks":{"football_private":{}},"healthcheck":{"test":["CMD","true"]}},"migrate":{"environment":{"APP_DB_NAME":"football_intelligence","APP_DB_USER":"football_app"}},"api":{"networks":{"football_edge":{},"football_private":{},"football_egress":{}},"healthcheck":{"test":["CMD","true"]}},"worker":{"networks":{"football_private":{},"football_egress":{}}},"frontend":{"networks":{"football_edge":{},"football_egress":{}},"healthcheck":{"test":["CMD","true"]}}},"volumes":{"postgres_data":{"name":"football_postgres_data"}},"networks":{"football_edge":{"internal":true},"football_private":{"internal":true},"football_egress":{}}}
JSON
    exit 0
  fi
